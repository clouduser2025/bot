...
async def process_newspaper(request: CityRequest):
    ...
    today = datetime.datetime.today().date()  # Use today's date dynamically
    
    # General pattern to match any filename with TOI, city, and date
    general_pattern = rf"TOI.*{city}.*{today.strftime('%d-%m')}\.pdf"
    old_pattern = rf"TOI {city} Tim\.\.\. {today.strftime('%d-%m')}\.pdf"
    new_pattern = rf"TOI {city} Times {today.strftime('%d-%m')}\.pdf"
    ...
    try:
        ...
        async for message in client.iter_messages(CHANNEL_USERNAME):
            if message.file and message.file.name:
                if re.search(general_pattern, message.file.name, re.IGNORECASE):
                    file_name = message.file.name
                    message_to_download = message
                    logger.info(f"Matched with general pattern: {file_name}")
                elif re.search(old_pattern, message.file.name, re.IGNORECASE):
                    file_name = message.file.name
                    message_to_download = message
                    logger.info(f"Matched with old pattern: {file_name}")
                elif re.search(new_pattern, message.file.name, re.IGNORECASE):
                    file_name = message.file.name
                    message_to_download = message
                    logger.info(f"Matched with new pattern: {file_name}")
        ...
        if file_name:
            steps_output["steps"].append({
                ...
                "status": "success",
                "filename": file_name,
                "matched_pattern": "general" if re.search(general_pattern, file_name, re.IGNORECASE) else "old" if re.search(old_pattern, file_name, re.IGNORECASE) else "new"
            })
            logger.info(f"Found PDF with {'general' if re.search(general_pattern, file_name, re.IGNORECASE) else 'old' if re.search(old_pattern, file_name, re.IGNORECASE) else 'new'} pattern: {file_name}")
        else:
            steps_output["steps"].append({
                "step": "Finding PDF",
                "status": "failed",
                "error": f"No PDF found for {city} on {today.strftime('%d-%m')} with patterns {general_pattern}, {old_pattern}, or {new_pattern}"
            })
            logger.warning(f"No PDF found matching patterns: {general_pattern}, {old_pattern}, or {new_pattern}")
